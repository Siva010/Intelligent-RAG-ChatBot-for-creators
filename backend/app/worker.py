...
from app.config import settings, normalized_redis_url_for_celery, redis_ssl_connection_kwargs
...
celery_broker_url = normalized_redis_url_for_celery(settings.redis_url)
...
def analyze_task(task_id: str, url_a: str, url_b: str, session_id: str):
    ...
    try:
        with ThreadPoolExecutor(max_workers=2) as ex:
            ...
            data_a, _ = fut_a.result()
            data_b, _ = fut_b.result()

        # Always index: ingest cache does not imply vectors exist (fresh deploy / empty Chroma).
        try:
            with ThreadPoolExecutor(max_workers=2) as ex:
                futures = [
